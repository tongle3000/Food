...
import { Categorys, IList } from '../../types';
...
interface IProps extends Categorys {
	navigation: RootStackNavigation;
	setCategorys:(res: any)=> void;
	getData:()=>void
}



export default class TabTwoScreen extends React.Component<IProps> {
			...
			<View style={styles.container}>
				...
				<ScrollView style={styles.content}>
					<View style={styles.categoryContainer}>
						...
						{/* <Text>{JSON.stringify(this.props.categorys)}s</Text> */}
						{/* {alert(this.state.categorys)} */}
						{
							this.props.categorys.map((item: IList) => {
								return (
									<TouchableWithoutFeedback onPress={()=>this.ItemClick(item.id, item.title)} key={item.id}>
										<View style={styles.item}>
											<Image style={styles.image} source={{ uri: item.image }} />
											<Text style={styles.ItemText}>{item.title}thunk</Text>
										</View>
	...
	componentDidMount() {
		this.props.getData()
		// 下面代码放进了 actionCreate getCategorysData()方法里,通过下面 getData()方法 dispatch actionCreate 里的方法就可以了
